...
fun ShadowsocksEncryptionMethod.config(pipeline: ChannelPipeline, password: String) {
    when (this) {
        ...
        ShadowsocksEncryptionMethod.AES256CFB -> {
            pipeline.addLast(ShadowSocksInboundDecoder(password, AES256CFBJceCipherProvider()))
            pipeline.addLast(ShadowSocksOutboundEncoder(password, AES256CFBJceCipherProvider()))
        }
